    ...
    d[zona].forEach(n=>{
      ...
      if(esZonaPromo){
        ...
        if(destLbl){
          ...
          s.title = n + ' — de ' + eq + ' a ' + dests.map(d=>d==='1ER EQUIPO'?'Primer Equipo':d).join(' y ');
          c.appendChild(s);
        }
      }
      cw.appendChild(c);
...
function eqsDeNombre(d, nombre){
  ...
  const eqs = EQUIPOS.filter(eq=>{
    ...
    const activoAqui = zonasChequear.some(z=>(data[d]?.[eq]?.[z]||[]).includes(nombre));
    if(activoAqui) return true;
    // Un jugador promocionado a un ÚNICO destino ya no cuenta como "presente" en su
    // equipo de origen (se fue de verdad, esto es normal y no debe contar como
    // "multi"). PERO si está a la vez en 2+ destinos (triplicado con el botón
    // "Triplicar"), su equipo de origen SÍ vuelve a contar como una ubicación más —
    // si no, un jugador con 2 destinos reales solo sumaba 2 en total y salía marcado
    // como "doblado" (rojo) en vez de "triplicado" (negro).
    const destinos = getDestinos(d, eq, nombre);
    return destinos.length >= 2;
  });
